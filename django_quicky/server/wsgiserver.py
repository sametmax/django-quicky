    ...
    def send_headers(self):
        ...
        try:
            ...
            if not isinstance(v, str):
                raise TypeError(
                          "WSGI response header value %r is not a string.")
            else:
                raise
        buf.append("\r\n")
        self.sendall("".join(buf))
...
##          'SSL_SERVER_V_START': Validity of server certificate (start time),
##          'SSL_SERVER_V_END': Validity of server certificate (end time),
            })
